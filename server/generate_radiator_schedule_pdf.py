...
def create_radiator_schedule_pdf(data, output_filename):
    ...
    legend_rows = [[
        Paragraph(
            "<font color='#1e40af'><b>\u25a0 Existing</b></font> = Existing radiator retained",
            ParagraphStyle('l', parent=styles['Normal'], fontSize=8)),
        Paragraph(
            "<font color='#15803d'><b>\u25a0 New</b></font> = New radiator to be installed",
            ParagraphStyle('l', parent=styles['Normal'], fontSize=8)),
        Paragraph(
    ...
    legend_row2 = [[
        Paragraph(
            "<font color='#9ca3af'><b>\u25a0 Replace</b></font> = Existing radiator to be replaced — 0 W contribution",
            ParagraphStyle('l', parent=styles['Normal'], fontSize=8)),
        Paragraph('', ParagraphStyle('l', parent=styles['Normal'], fontSize=8)),
        Paragraph('', ParagraphStyle('l', parent=styles['Normal'], fontSize=8)),
    ]]
    ...
    legend2 = Table(legend_row2, colWidths=[5.8*cm, 5.8*cm, 5.8*cm])
    legend2.setStyle(TableStyle([
        ('BACKGROUND',   (0, 0), (-1, -1), GRAY_LIGHT),
        ('TOPPADDING',   (0, 0), (-1, -1), 5),
        ('BOTTOMPADDING',(0, 0), (-1, -1), 5),
        ('LEFTPADDING',  (0, 0), (-1, -1), 8),
    ...
    for room in rooms:
        ...
        if schedule:
            ...
            spec_style_strike = ParagraphStyle('specs', parent=styles['Normal'],
                fontSize=8, leading=10, textColor=colors.HexColor('#9ca3af'))
            for item in schedule:
                is_ufh      = item.get('isUFH', False)
                status      = item.get('emitterStatus', 'new') if not is_ufh else 'ufh'
                is_replace  = status == 'existing_replace'
                dt50        = item.get('outputDt50')

                if is_ufh:
                    status_label = 'UFH'
                elif status == 'existing_retained':
                ...
                # Strike through spec text for replaced radiators to signal removal
                s_style = spec_style_strike if is_replace else spec_style
                sched_rows.append([
                    status_label,
                    Paragraph(item.get('spec', 'N/A'), s_style),
                    item.get('connectionType', 'BOE'),
                    str(item.get('quantity', 1)),
                    '—' if (is_ufh or dt50 is None) else f"{dt50:.0f} W",
                    '0 W (replaced)' if is_replace else f"{item.get('outputAtDesign', 0):.0f} W",
                ])
            ...
            GRAY_ROW = colors.HexColor('#f3f4f6')
            for i, item in enumerate(schedule, 1):
                status = item.get('emitterStatus', 'new') if not item.get('isUFH') else 'ufh'
                ...
                elif status == 'existing_replace':
                    st.append(('BACKGROUND', (0, i), (-1, i), GRAY_ROW))
                    st.append(('TEXTCOLOR',  (0, i), (0, i),  GRAY_MID))
                    st.append(('TEXTCOLOR',  (2, i), (-1, i), GRAY_MID))
                    st.append(('FONTNAME',   (0, i), (0, i),  'Helvetica-Bold'))
                elif status == 'existing_retained':
