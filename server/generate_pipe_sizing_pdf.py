...
def generate_pipe_sizing_pdf(data, output_path):
    ...
    if flow_rows_data:
        ...
        fr_hdr = [
            ...
            Paragraph('Output/unit (W)',  ParagraphStyle('FH', parent=styles['Normal'], fontSize=8, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
            Paragraph('Flow/unit (l/h)', ParagraphStyle('FH', parent=styles['Normal'], fontSize=8, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
            Paragraph('Room total (W)',   ParagraphStyle('FH', parent=styles['Normal'], fontSize=8, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
        ]
        fr_rows = [fr_hdr]
        ...
        for r in flow_rows_data:
            fr_rows.append([
                ...
                f"{r.get('outputW', 0)}",
                f"{r.get('flowLh', 0):.1f}",
                f"{r.get('outputTotalW', r.get('outputW', 0))}",
            ])
        # Totals row — sum room totals for overall output
        tot_w = sum(r.get('outputTotalW', r.get('outputW', 0)) for r in flow_rows_data)
        fr_rows.append(['TOTAL', '', '', 'per unit', 'per unit', f"{tot_w}"])
