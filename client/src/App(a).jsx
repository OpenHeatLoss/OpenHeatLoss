...
function App() {
  ...
  // Boot sequence: check for an existing anonymous session first.
  // If found (or newly created), load that project directly.
  // If the anonymous endpoint fails (e.g. network error), fall back
  // to the normal dashboard so the app is still usable.
  useEffect(() => {
    const bootAnonymous = async () => {
      try {
        // Check whether this browser already has an anonymous project
        let res = await fetch('/api/anonymous/project');
        let { projectId } = await res.json();

        // No existing project — create one now
        if (!projectId) {
        ...
        if (projectId) {
          ...
          // Fallback: couldn't create an anonymous project — load dashboard
          loadProjects();
        }
      } catch (err) {
      ...
      {isAnonymous && (
        <div className="bg-amber-50 border-b border-amber-300 px-4 py-3">
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
            ...
            <button
              onClick={() => alert('Registration coming soon — your project will be preserved when you sign up.')}
              className="shrink-0 bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold px-4 py-1.5 rounded transition"
            >
              Register free
            </button>
          </div>
      ...
      <div className="bg-blue-600 text-white p-4 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={isAnonymous
                ? () => alert('Register free to save your project. Your work will be preserved when you sign up.')
                : saveProject}
              disabled={saving}
