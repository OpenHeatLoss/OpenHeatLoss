...
export default function MCS020SoundCalculator({ project, onUpdate, onSave }) {
  ...
  // Fingerprint of current inputs to detect changes since last calculation
  const inputFingerprint = JSON.stringify({
    mcsHeatPumpSoundPower: project.mcsHeatPumpSoundPower,
    assessments: (project.mcsSoundAssessments || []).map(p => ({
      id: p.id, soundPowerLevel: p.soundPowerLevel, directivity: p.directivity,
  ...
  // Sync state when project data changes (e.g., when loading a different project)
  useEffect(() => {
    let assessments = project.mcsSoundAssessments;
    
    // If it's a string, parse it
    if (typeof assessments === 'string' && assessments.length > 0) {
      try {
        assessments = JSON.parse(assessments);
      } catch (e) {
        console.error('Failed to parse MCS sound assessments:', e);
        return;
      }
    }
    
    // Always update if it's an array (even if empty)
    if (assessments && Array.isArray(assessments)) {
      setAssessmentPositions(assessments.length > 0 ? assessments : [
        {
          id: 1,
          date: new Date().toISOString().split('T')[0],
          description: '',
  ...
  }, [project.mcsSoundAssessments, project.mcsHeatPumpSoundPower]);
  ...
  const handleCalculate = () => {
    ...
    // Save snapshot with timestamp and fingerprint for audit trail
    const newFingerprint = JSON.stringify({
      mcsHeatPumpSoundPower: project.mcsHeatPumpSoundPower,
      assessments: updated.map(p => ({
        id: p.id, soundPowerLevel: p.soundPowerLevel, directivity: p.directivity,
        distance: p.distance, barrierType: p.barrierType, lineOfSight: p.lineOfSight
      }))
    });
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const calcDate = snapshot ? new Date(snapshot.calculatedAt).toLocaleDateString('en-GB', {
  ...
  const updatePosition = (id, field, value) => {
    const newPositions = assessmentPositions.map(pos => 
  ...
  return (
    <div className="space-y-6">
      ...
      {inputsChangedSinceSnapshot && (
        <div className="bg-amber-50 border-l-4 border-amber-500 rounded p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700">The saved results below may no longer be accurate. Click Recalculate to update.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 flex-wrap">
        <button
          ...
          className={`text-white px-6 py-3 rounded font-semibold transition disabled:bg-gray-400 disabled:cursor-not-allowed ${
            inputsChangedSinceSnapshot ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700'
          }`}
        >
          {snapshot
            ? '⟳ Recalculate & Save'
            : 'Calculate & Save Sound Levels'}
        </button>
      ...
      {snapshot && (
        <>
          <div className={`border rounded-lg p-3 flex justify-between items-center ${inputsChangedSinceSnapshot ? 'bg-amber-50 border-amber-400' : 'bg-green-50 border-green-500'}`}>
            <span className={`font-semibold ${inputsChangedSinceSnapshot ? 'text-amber-800' : 'text-green-800'}`}>
              {inputsChangedSinceSnapshot ? '⚠ Inputs changed — results below may be outdated' : '✓ Assessment saved to project'}
            </span>
            <span className="text-xs text-gray-500 text-right">
              <span className="block font-semibold text-gray-700">Calculation date</span>
              {new Date(snapshot.calculatedAt).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}
              {' '}{new Date(snapshot.calculatedAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
